import { songs as t } from "../data/songs.js";
import { albums as e } from "../data/albums.js";
import { Categories as n } from "../data/category.js";
import { getAverageColor as a } from "../helpers/getAverageColor.js";
...
let processCurSongPlaying = () => {
    ...
    if (a)
      try {
        ...
        let r = Array.from(t).indexOf(a);
        -1 !== r &&
          ((e[r].style.display = "inline-block"),
  ...
  app = {
    ...
    searchF: function () {
      ...
      `),
              ...
              let r = n.map(
                (e, n) => `
                  <div class="song ${
                    n === t.currentIndex ? "active" : ""
                  }" data-index="${n}">
                    <div class="thumb" style="background-image: url('${
                      e.imagecover ? e.imagecover : ""
                    }')"></div>
                    <div class="body">
            ...
            i.forEach((e) => {
              e.onclick = function () {
                (t.currentIndex = Number(e.dataset.index)),
                  t.loadCurrentSong(),
    ...
    render__one: function () {
      let t = this.songs.map((t, e) => {
        if (void 0 != t)
          return `
              <div class="song ${
                e === this.currentIndex ? "active" : ""
              }" data-index="${e}">
                  <div class="thumb"
                      style="background-image: url('${
                        t.imagecover ? t.imagecover : ""
                      }')">
                  </div>
                  <div class="body">
    ...
    handleEvents: function () {
      ...
      e.pause(),
        (audio.ontimeupdate = function t() {
          if (!isNaN(audio.duration)) {
            let e = Math.floor(audio.currentTime / 60),
              ...
              i = `${e}:${n < 10 ? "0" : ""}${n}`,
              o = `${a}:${r < 10 ? "0" : ""}${r}`;
            (currentTime.innerHTML = `<span>${i}</span>`),
              (remainTime.innerHTML = `<span>${o}</span>`);
          }
          if (audio.duration) {
        ...
        (nextBtn.onclick = function e() {
          t.isRandom ? t.playRandomSong() : t.nextSong(),
            audio.play(),
            t.render__one(),
            t.scrollToActiveSong();
        }),
        (prevBtn.onclick = function e() {
          t.isRandom ? t.playRandomSong() : t.prevSong(),
            audio.play(),
            t.render__one(),
            t.scrollToActiveSong();
        }),
        (randomBtn.onclick = function e() {
          (t.isRandom = !t.isRandom),
            t.setConfig("isRandom", t.isRandom),
            randomBtn.classList.toggle("active", t.isRandom);
        }),
        (repeatBtn.onclick = function e() {
          (t.isRepeat = !t.isRepeat),
            t.setConfig("isRepeat", t.isRepeat),
            repeatBtn.classList.toggle("active", t.isRepeat);
        }),
        ...
        (playlist.onclick = function e(n) {
          ...
          (a || !n.target.contains(".favoriteSong")) &&
            (a &&
              ((t.currentIndex = Number(a.dataset.index)),
              t.loadCurrentSong(),
              t.render__one(),
              audio.play()),
            n.target.closest(".favoriteSong") &&
              n.target.classList.toggle("active"));
        });
    },
    ...
    loadCurrentSong: function () {
      if (this.currentSong) {
        ...
        let t = new Image();
        t
          ? ((t.src = app.currentSong.imagecover),
            t.setAttribute("crossOrigin", "anonymous"),
            (t.onload = function () {
              let { R: e, G: n, B: r } = a(t, 4);
              ($(".side-bar").style.background = `rgb(${e}, ${n},${r})`),
                ($(".main").style.background = `rgb(${e}, ${n},${r})`),
                ($("#navbarFixed").style.background = `rgb(${e}, ${n},${r})`);
            }))
    ...
    playRandomSong: function () {
      let t;
      do t = Math.floor(Math.random() * this.songs.length);
